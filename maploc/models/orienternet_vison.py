...
class OrienterNet(BaseModel):
    ...
    def exhaustive_voting(self, f_bev, f_map, valid_bev, confidence_bev=None):
        ...
        return scores
    def visualize_depth_distribution_heatmap(self,scales):
         # 对深度分布参数进行求和
        merged_scales = torch.mean(scales, dim=-1, keepdim=True)  # 在最后一个维度上求和
        
        # 将 PyTorch 张量移动到 CPU 上，并转换为 NumPy 数组
        merged_scales_cpu = merged_scales.cpu().numpy()
        
        plt.figure(figsize=(10, 8))
        plt.imshow(merged_scales_cpu[0, :, :, 0], cmap='jet', aspect='auto')  
        plt.colorbar()
