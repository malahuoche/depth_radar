...
class FeatureExtractor(BaseModel):
    ...
    mean = [0.485, 0.456, 0.406, 0]  # 修改 mean 为 4 通道
    ...
    def build_encoder(self, conf):
        ...
        if conf.encoder.startswith("resnet"):
            ...
            with torch.no_grad():
                new_conv1.weight[:, :3, :, :] = conv1.weight  # 复制原始权重
                new_conv1.weight[:, 3, :, :] = conv1.weight[:, 0, :, :]  # 初始化新通道的权重
    ...
    def _forward(self, data):
        image = data["image_d"]
        #这里把depth加进去
        # image_d = torch.cat((image, depth), dim=1)
        device = image.device
        # 将均值和标准差转移到同一设备
        mean_ = self.mean_.to(device)
        std_ = self.std_.to(device) 
        self.decoder.to(device) 
            # 对图像进行归一化
        image = (image - mean_[None, :, None, None]) / std_[None, :, None, None]
