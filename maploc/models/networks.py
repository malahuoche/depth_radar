import torch
from .net_utils import Conv2d,ResNetBlock,activation_func, DecoderBlock,FullyConnected
import torchvision
...
class ResNetEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        activation_func = torch.nn.LeakyReLU(negative_slope=0.20, inplace=True)
...
class FusionNetEncoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if fusion_type == 'add':
            self.conv1_project = Conv2d(
                n_filters_encoder_depth[filter_idx],
                n_filters_encoder_image[filter_idx],
                kernel_size=1,
                stride=1,
                weight_initializer=weight_initializer,
                activation_func=activation_func('linear'),
                use_batch_norm=use_batch_norm)

        elif fusion_type == 'weight':

            self.conv1_weight = Conv2d(
                n_filters_encoder_depth[filter_idx],
                n_filters_encoder_depth[filter_idx],
                kernel_size=3,
                stride=1,
                weight_initializer=weight_initializer,
                activation_func=activation_func('sigmoid'),
                use_batch_norm=use_batch_norm)
...
class RadarNetV1Encoder(torch.nn.Module):
    ...
    def __init__(self,
                 input_channels_image=3,
                 input_channels_depth=3,
                 input_patch_size_image=(2048, 288),
                 n_filters_encoder_image=[32, 64, 128, 128, 128],
    ...
    def forward(self, image, points, b_boxes):
        ...
        # print("bounding_boxes 维度:", b_boxes.shape)
        # Define dimensions
        ...
        latent_image, skips_image = self.encoder_image(image) #[1,128,8,8]

        # bounding_boxes_list = [b_boxes[0][idx] for idx in range(b_boxes.shape[1])]
        # 创建一个空列表来存储转换后的元素
        b_boxes_list = []
        B = b_boxes.shape[0]
        # 遍历每个B，并将其添加到列表中
        for i in range(B):
            b_boxes_list.append(b_boxes[i])
        b_boxes = b_boxes_list
        # print(np.array(bounding_boxes_list).shape)
        ...
        latent = torch.cat([latent_image_pooled, latent_depth], dim=1)#depth:[1,128,50888,4] image[12722,128,4,4]
...
class MultiScaleDecoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        # activation_func = activation_func(activation_func)
        activation_func = torch.nn.LeakyReLU(negative_slope=0.20, inplace=True)
        # output_func = activation_func(output_func)
        output_func = None
